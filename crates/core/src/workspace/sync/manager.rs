...
///
/// Owned by `NetManager` alongside DeviceManager and PairingManager. Accesses
/// workspace DBs, the workspace registry, `YDocManager`, and identity through
/// [`Arc<AppCore>`] — no direct Tauri dependency.
...
impl SyncManager {
    pub fn new(core: Arc<AppCore>, client: AppNetClient) -> Self {
        ...
        let flush_handle = pending_buffer.spawn_flush_task(Arc::clone(&core));
        Self {
            core,
            client,
    ...
    pub async fn on_paired_peer_connected(self: &Arc<Self>, peer_id: PeerId) {
        ...
        for ws in &workspaces {
            self.sync_workspace(peer_id, ws.info.id).await;
        }
    }
    ...
    pub async fn handle_ctrl_message(self: &Arc<Self>, source: PeerId, msg: super::CtrlMessage) {
        match msg {
            super::CtrlMessage::WorkspaceOpened { uuid } => {
                if self.core.get_workspace(&uuid).await.is_some() {
    ...
    /// Subscribe + full sync for a workspace. In the core layer there is no
    /// distinction between window-bound and sync-only workspaces (that was a
    /// Tauri-specific concept); all open workspaces both subscribe to the
    /// GossipSub topic and participate in full sync.
    ...
    pub async fn handle_ws_gossip_update(
        ...
        let Some(ws) = self.core.get_workspace(&workspace_uuid).await else {
            // Workspace not open locally — buffer (still lets us flush on
            // a future open_workspace).
            if let Some((ws_id, _source_peer, overflow)) = self
                .pending_buffer
                ...
                .await
            {
                for update in &overflow {
                    if let Err(e) =
                        doc_sync::apply_remote_update(&self.core, ws_id, doc_uuid, update).await
                    {
    ...
    /// Find workspace UUID and rel_path for a document by searching all open workspaces.
    ...
    async fn run_sv_compensation(self: &Arc<Self>) {
        // Gather every open doc across every open workspace, keyed by workspace.
        let workspaces = self.core.list_workspaces().await;
        ...
        // Reach DeviceManager via the live NetManager (if any).
