...
pub struct OsInfo {
    /// 用户自定义设备名称（可选，通过 agent_version 的 `name=` 字段传播）
...
impl OsInfo {
    ...
    /// 编码为 agent_version 格式。
    /// 有 name 时：`swarmnote/{version}; name={name}; host={hostname}; os=...; platform=...; arch=...`
    /// 无 name 时：`swarmnote/{version}; host={hostname}; os=...; platform=...; arch=...`
...
mod tests {
    ...
    #[test]
    fn os_info_agent_version_with_name() {
        let info = OsInfo {
