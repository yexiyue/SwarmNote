...
pub async fn apply_remote_update(
    ...
    if let Some(result) = ydoc_mgr.apply_sync_update(app, &doc_uuid, update).await {
        return result;
    }

    // Document not open → temporary Doc path
    sync_closed_doc(app, workspace_uuid, doc_uuid, update).await
}
...
async fn sync_closed_doc(
    ...
    let db: &sea_orm::DatabaseConnection = guard.conn();
...
pub async fn sync_via_state_vector(
    ...
    let sv = load_local_state_vector(app, workspace_uuid, doc_id).await?;
    ...
    match response {
        AppResponse::Sync(SyncResponse::Updates { doc_id: _, updates }) => {
            if !updates.is_empty() {
                apply_remote_update(app, workspace_uuid, doc_id, &updates).await?;
...
pub async fn handle_state_vector_request(
    ...
    let diff = compute_update_for_peer(app, workspace_uuid, doc_id, remote_sv_bytes).await?;
...
pub async fn handle_full_sync_request(
    ...
    let full_state = load_full_doc_state(app, workspace_uuid, doc_id).await?;
...
pub async fn apply_deletion(
    ...
    let peer_id_str = identity.peer_id().unwrap_or_default();
...
pub async fn resolve_path_conflict(
    ...
    if local_is_winner {
        ...
        rename_local_doc(app, workspace_uuid, local_doc_id, rel_path, &new_path).await?;
        sync_via_full_pull(
            app,
            client,
            peer_id,
...
async fn rename_local_doc(
    ...
    let ydoc_mgr = app.state::<YDocManager>();
    ydoc_mgr.rename_doc_by_uuid(&doc_id, new_rel_path);
...
async fn load_local_state_vector(
    ...
    let ydoc_mgr = app.state::<YDocManager>();
    if let Some(sv) = ydoc_mgr.get_state_vector(&doc_id).await {
        return Ok(sv);
    }
...
async fn compute_update_for_peer(
    ...
    let ydoc_mgr = app.state::<YDocManager>();
    if let Some(update) = ydoc_mgr.encode_diff_for_sv(&doc_id, &remote_sv).await {
        return Ok(update);
    }
...
async fn load_full_doc_state(
    ...
    // Try YDocManager first
    let ydoc_mgr = app.state::<YDocManager>();
    if let Some(state) = ydoc_mgr.encode_full_state(&doc_id).await {
        return Ok(state);
    }
...
pub(crate) async fn get_workspace_path(
    ...
    let info = ws_state
        .get(&workspace_uuid)
        .await
        .ok_or_else(|| AppError::NoWorkspaceDb)?;
