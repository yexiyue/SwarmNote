...
fn priority_key(action: &SyncAction, app: &AppHandle) -> (u8, i64) {
    ...
    if ydoc_mgr.is_doc_open(&doc_id) {
        (0, 0)
    } else {
...
pub async fn full_sync(
    ...
    let _ = app.emit(
        ...
        serde_json::json!({
            ...
            "workspaceUuid": workspace_uuid.to_string(),
        }),
    );
    ...
    for action in &actions {
        ...
        if let Err(e) = execute_action(&app, &client, peer_id, workspace_uuid, action).await {
    ...
    let _ = app.emit(
        ...
        serde_json::json!({
            "peerId": peer_id.to_string(),
            "workspaceUuid": workspace_uuid.to_string(),
            "result": if cancel.is_cancelled() { "cancelled" } else { "success" },
        }),
    );
...
async fn execute_action(
    ...
    match action {
        SyncAction::StateVectorExchange { doc_id } => {
            doc_sync::sync_via_state_vector(app, client, peer_id, workspace_uuid, *doc_id).await?;
            // Asset sync follows doc sync
            if let Some((_, rel_path)) = find_doc_rel_path(app, workspace_uuid, *doc_id).await {
        ...
        SyncAction::FullPull { doc_id, rel_path } => {
            doc_sync::sync_via_full_pull(app, client, peer_id, workspace_uuid, *doc_id, rel_path)
                .await?;
            asset_sync::sync_doc_assets(app, client, peer_id, workspace_uuid, *doc_id, rel_path)
                .await
                .ok();
        }
        ...
        SyncAction::ApplyDeletion {
            ...
            doc_sync::apply_deletion(app, workspace_uuid, *doc_id, rel_path, *lamport_clock)
                .await?;
        }
...
async fn find_doc_rel_path(
    ...
    use entity::workspace::documents;
    let db_state = app.state::<DbState>();
    let guard = db_state.workspace_db(&workspace_uuid).await.ok()?;
