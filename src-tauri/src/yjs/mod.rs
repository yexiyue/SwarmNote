...
/// Must match the frontend `@swarmnote/editor` default fragment name
/// (see `packages/editor/src/createEditor.ts`).
pub(crate) const FRAGMENT_NAME: &str = "document";

/// Create an empty Y.Doc with a registered Y.Text fragment and Utf16 offset.
///
/// All Y.Docs **must** use `OffsetKind::Utf16` to match JavaScript yjs —
/// the yrs default `OffsetKind::Bytes` causes index overflow panics on CJK text.
...
pub(crate) fn doc_to_markdown(doc: &Doc) -> String {
...
///
/// Should only be called on a freshly created Doc (empty Y.Text). If the Y.Text
/// already has content, this is a no-op — caller should use [`replace_doc_content`]
/// instead.
pub(crate) fn fill_doc_with_markdown(doc: &Doc, md: &str) {
...
/// insert/delete ops that preserve CRDT history (concurrent edits are merged,
/// not overwritten).
///
/// Diff is computed at the UTF-16 code unit level, matching the Y.Text offset
/// semantics (`OffsetKind::Utf16`).
pub(crate) fn replace_doc_content(doc: &Doc, new_md: &str) {
    let text = doc.get_or_insert_text(FRAGMENT_NAME);
    let mut txn = doc.transact_mut();
...
mod tests {
    ...
    fn fill_is_idempotent_when_nonempty() {
        ...
        fill_doc_with_markdown(&doc, "second"); // no-op — first had content
    ...
    fn concurrent_edits_merge_via_replace() {
        ...
        // B receives an external file change: "hello!"
        replace_doc_content(&doc_b, "hello!");

        // Now A's update arrives at B
        apply_update_to_doc(&doc_b, &update_from_a, "from A").unwrap();

        // Both edits should survive. The exact merge order is CRDT-defined, but
        // the result must contain both " world" (from A) and "!" (from external).
